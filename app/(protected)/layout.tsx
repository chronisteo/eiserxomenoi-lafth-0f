...
function createCookieClient(supabaseKey: string) {
...
export default async function ProtectedLayout({ children }: { children: ReactNode }) {
  const supabaseAuthClient = createCookieClient(SUPABASE_ANON_KEY || "")
  const {
    data: { session },
  ...
  if (!ALLOWED_EMAIL) {
    throw new Error("Allowed email is not configured. Set SUPABASE_ALLOWED_EMAIL or ALLOWED_EMAIL.")
  }
  ...
  if (!SUPABASE_SERVICE_ROLE_KEY) {
    throw new Error("SUPABASE_SERVICE_ROLE_KEY is required for data operations")
  }
